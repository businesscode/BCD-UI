...
<xsl:template name="date:get-month-length-sum">
  <xsl:param name="month-no"/>
  <xsl:choose>
    <xsl:when test="$month-no = 1">0</xsl:when>
    <xsl:when test="$month-no = 2">31</xsl:when>
    <xsl:when test="$month-no = 3">59</xsl:when>
    <xsl:when test="$month-no = 4">90</xsl:when>
    <xsl:when test="$month-no = 5">120</xsl:when>
    <xsl:when test="$month-no = 6">151</xsl:when>
    <xsl:when test="$month-no = 7">181</xsl:when>
    <xsl:when test="$month-no = 8">212</xsl:when>
    <xsl:when test="$month-no = 9">243</xsl:when>
    <xsl:when test="$month-no = 10">273</xsl:when>
    <xsl:when test="$month-no = 11">304</xsl:when>
    <xsl:when test="$month-no = 12">334</xsl:when>
    <xsl:when test="$month-no = 13">365</xsl:when>
               ...
               <xsl:choose>
                  ...
                  <xsl:when test="substring($start-dt, 8, 1) = '-' and
                     ...
                     <xsl:if test="$start-day &lt;= 31 and $end-day &lt;= 31">
                        ...
                        <!-- Changed, because the document-function doesn't work with empty string -->
                        <xsl:variable name="start-month-days">
                          <xsl:call-template name="date:get-month-length-sum">
                            <xsl:with-param name="month-no"><xsl:value-of select="$start-month"/></xsl:with-param>
                          </xsl:call-template>
                        </xsl:variable>
                        <!-- end of change -->
                        ...
                        <xsl:variable name="end-month-days">
                          <xsl:call-template name="date:get-month-length-sum">
                            <xsl:with-param name="month-no"><xsl:value-of select="$end-month"/></xsl:with-param>
                          </xsl:call-template>
                        </xsl:variable>
                        <!-- end of change -->
