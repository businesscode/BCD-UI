...
<xsl:template name="date:get-month-length">
  <xsl:param name="month-no"/>
  <xsl:choose>
    <xsl:when test="$month-no = 1">31</xsl:when>
    <xsl:when test="$month-no = 2">28</xsl:when>
    <xsl:when test="$month-no = 3">31</xsl:when>
    <xsl:when test="$month-no = 4">30</xsl:when>
    <xsl:when test="$month-no = 5">31</xsl:when>
    <xsl:when test="$month-no = 6">30</xsl:when>
    <xsl:when test="$month-no = 7">31</xsl:when>
    <xsl:when test="$month-no = 8">31</xsl:when>
    <xsl:when test="$month-no = 9">30</xsl:when>
    <xsl:when test="$month-no = 10">31</xsl:when>
    <xsl:when test="$month-no = 11">30</xsl:when>
    <xsl:when test="$month-no = 12">31</xsl:when>
...
<xsl:template name="date:_add-days">
   ...
   <!-- workaround, because the document('') function doesn't work with empty argument -->
   <xsl:variable name="days-in-month-without-leap">
     <xsl:call-template name="date:get-month-length">
       <xsl:with-param name="month-no"><xsl:value-of select="number($month)"/></xsl:with-param>
     </xsl:call-template>
   </xsl:variable>
   <!-- end of workaround -->
   <xsl:variable name="days-in-month">
      <xsl:choose>
         <xsl:when test="$month = 2 and $leap">
            <xsl:value-of select="number($days-in-month-without-leap) + 1" />
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="number($days-in-month-without-leap)" />
         </xsl:otherwise>
      </xsl:choose>
   </xsl:variable>
                     ...
                     <xsl:variable name="days-in-new-month">
                        <xsl:choose>
                           ...
                           <xsl:otherwise>
                             <!-- workaround, because the document('') function doesn't work with empty argument -->
                             <xsl:call-template name="date:get-month-length">
                               <xsl:with-param name="month-no"><xsl:value-of select="number($month) - 1"/></xsl:with-param>
                             </xsl:call-template>
                             <!-- end of workaround -->
                           </xsl:otherwise>
                        </xsl:choose>
