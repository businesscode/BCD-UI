<?xml version="1.0" encoding="UTF-8"?>
<!--
  Copyright 2010-2017 BusinessCode GmbH, Germany

  Licensed under the Apache License, Version 2.0 (the "License");
  you may not use this file except in compliance with the License.
  You may obtain a copy of the License at

  http://www.apache.org/licenses/LICENSE-2.0

  Unless required by applicable law or agreed to in writing, software
  distributed under the License is distributed on an "AS IS" BASIS,
  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  See the License for the specific language governing permissions and
  limitations under the License.
-->
<xsl:stylesheet version="1.0"
  xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:f="http://www.businesscode.de/schema/bcdui/filter-1.0.0"
  xmlns:wrs="http://www.businesscode.de/schema/bcdui/wrs-1.0.0"
  xmlns:xp="http://www.businesscode.de/schema/bcdui/xsltParams-1.0.0"
  xmlns:exslt="http://exslt.org/common"
  xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  xmlns:generator="urn(bcd-xsltGenerator)">

  <msxsl:script language="JScript" implements-prefix="exslt">this['node-set'] = function (x) { return x; }</msxsl:script>

  <!--
    (re)order columns and rows and drop columns of input WRS, see xp:OrderRowsAndColumns
    -->

  <xsl:output method="xml" version="1.0" encoding="UTF-8" indent="no" />

  <!-- Parameter model  -->
  <!-- (DOM) Parameter model according to xmlns http://www.businesscode.de/schema/bcdui/xsltParams-1.0.0 -->
  <xsl:param name="paramModel" select="/*[0=1]"/>
  <!-- (String) Optional specific parameter set ID  -->
  <xsl:param name="paramSetId"/>
  <!-- (NodeSet) As parameter or as default or specific parameter set from parameter model-->
  <xsl:param name="paramSet" select="$paramModel//xp:OrderRowsAndCols[@paramSetId=$paramSetId or not(@paramSetId) and not($paramSetId)]"/>

  <!-- Individual parameters -->
  <!-- (Node) wrs:Header/wrs:Columns element containing new columns and their order on output -->
  <xsl:param name="rowsOrder" select="$paramSet/xp:RowsOrder"/>
  <!-- Variable colsOrder can come directly from the parameters or can be calculated during template generation -->
  <generator:ColDimsOrdering/>
  <generator:ColsOrderVar/>
  <generator:RowFilterKey/>

  <xsl:variable name="doc" select="/*"/>

  <xsl:key name="colHeadById"  match="/*/wrs:Header/wrs:Columns/wrs:C" use="@id"/>
  <xsl:key name="colHeadByPos"  match="/*/wrs:Header/wrs:Columns/wrs:C" use="@pos"/>

  <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    root template
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
  <xsl:template match="/*">
    <xsl:copy>
      <xsl:apply-templates select="@*|node()" mode="ordering"/>
    </xsl:copy>
  </xsl:template>

  <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Copy 1:1
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
  <xsl:template match="@*|node()" mode="ordering">
    <xsl:copy><xsl:apply-templates select="@*|node()" mode="ordering"/></xsl:copy>
  </xsl:template>

  <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ordering header Columns
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
  <xsl:template match="/wrs:Wrs/wrs:Header/wrs:Columns" mode="ordering">
    <xsl:choose>
      <xsl:when test="$colsOrder/*">
        <xsl:variable name="wrsCols" select="."/>
        <xsl:copy>
          <xsl:copy-of select="@*"/>
          <xsl:for-each select="$colsOrder/*">
            <xsl:variable name="newPos" select="position()"/>
            <xsl:variable name="curCol" select="."/>
            <wrs:C>
              <xsl:variable name="srcNode" select="$wrsCols/wrs:C[@id=$curCol/@id]"/>
              <xsl:copy-of select="$srcNode/@*"/>
              <xsl:attribute name="pos"><xsl:value-of select="$newPos"/></xsl:attribute>
              <xsl:copy-of select="$srcNode/node()"/>
            </wrs:C>
          </xsl:for-each>
        </xsl:copy>
      </xsl:when>
      <xsl:otherwise>
        <xsl:copy-of select="."/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- ToDo: removing ValidationResult on column sort (later feature) -->
  <xsl:template match="wrs:ValidationResult" mode="ordering">
    <xsl:choose>
      <xsl:when test="$colsOrder/*">
        <xsl:comment>Validation removed by orderRowsAndCols</xsl:comment>
      </xsl:when>
      <xsl:otherwise>
        <xsl:copy-of select="."/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ordering Data/*
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
  <xsl:template match="wrs:Data" mode="ordering">
    <xsl:copy>
      <xsl:copy-of select="@*"/>
      <generator:ForEachRow>
        <generator:RowsOrdering/>
        <xsl:apply-templates select="." mode="ordering"/>
      </generator:ForEachRow>
    </xsl:copy>
  </xsl:template>

  <!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ordering Data/*/*
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
  <xsl:template match="wrs:Data/*" mode="ordering">
    <xsl:choose>
      <xsl:when test="$colsOrder/*">
        <xsl:copy>
          <xsl:copy-of select="@*"/>
          <xsl:variable name="wrsCols" select="*"/>
          <xsl:for-each select="$colsOrder/*">
            <xsl:variable name="colItem" select="."/>
            <xsl:for-each select="$doc">
              <wrs:C>
                <xsl:copy-of select="$wrsCols[self::wrs:C][number(key('colHeadById', $colItem/@id)/@pos)]/@*[name() != 'pos']"/>
                <xsl:value-of select="$wrsCols[self::wrs:C][number(key('colHeadById', $colItem/@id)/@pos)]/text()"/>
              </wrs:C>
              <xsl:if test="$wrsCols[self::wrs:O][number(key('colHeadById', $colItem/@id)/@pos)]">
                <wrs:O>
                  <xsl:copy-of select="$wrsCols[self::wrs:O][number(key('colHeadById', $colItem/@id)/@pos)]/@*[name() != 'pos']"/>
                  <xsl:value-of select="$wrsCols[self::wrs:O][number(key('colHeadById', $colItem/@id)/@pos)]/text()"/>
                </wrs:O>
              </xsl:if>
          </xsl:for-each>
          </xsl:for-each>
        </xsl:copy>
      </xsl:when>
      <xsl:otherwise>
        <xsl:copy-of select="."/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

</xsl:stylesheet>
