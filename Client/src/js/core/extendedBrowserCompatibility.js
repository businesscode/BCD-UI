...
if (bcdui.browserCompatibility.isWebKit || bcdui.browserCompatibility.isGecko) {
  ...
  bcdui.core.browserCompatibility.fixXslt = {
     ...
     * @param {boolean}     args.xsltGeneratedBy is generated XSLT
    */
...
//-----------------------------------------------------------------------------
  ...
  // cache already loaded and parsed sef json files
  window.XSLTProcessor.sefCache = new Map();

 /*
  * @param {Object}      args - An argument map containing the following elements:
  * @param {DomDocument} args.model The XSLT document the XSLTProcessor instance should be
  * @param {function}    args.callBack The callback function executed when the processor has been created. It takes the processor instance as argument
  * @param {boolean}     args.xsltGeneratedBy is a generated
  */
  bcdui.core.browserCompatibility.asyncCreateXsltProcessor =  function( args ) {
    ...
    // or we work on a generated xslt document
      ...
      proc.xsltGeneratedBy = args.xsltGeneratedBy
    }
      ...
      return SaxonJS.transform(params, "async")
        .then( r => {
          if(bcdui.config.isDebug) {
            let name = this.xsltGeneratedBy ?? args.stylesheetLocation;
            name = name.startsWith(location.origin) ? name.slice(location.origin.length) : name;
            name = name.startsWith(bcdui.config.contextPath) ? name.slice(bcdui.config.contextPath.length) : name;
            let text = "BCD-UI " + ( this.xsltGeneratedBy ? `transforming XSLT generated by ${name} to sef` : name);
            performance.measure(text, { start: t0, end: performance.now() });
            bcdui.log.trace( text );
          }
          return r;
        })
        .catch( res => {
          let name = this.xsltGeneratedBy ?? args.stylesheetLocation;
          bcdui.log.error( "BCD-UI " + ( this.xsltGeneratedBy ? `transforming XSLT generated by ${name} to sef` : name) + " failed", params, res );
        });
      })
    .catch( res => console.error("Could not load stylesheet for", args.stylesheetLocation, res) );
  }
  ...
  XSLTProcessor.prototype.asyncTransform = function(sourceDoc)
  {
    ...
    if (!this.stylesheetNode)
      resultPromise = this.transformWithCache({stylesheetLocation: this.sefJson, sourceDoc: sourceDoc});
    ...
    // if we have a stylesheetNode which was generated by a loaded xslt (e.g. single cube chain xslts), we also have a belonging sef.json file 
      ...
      resultPromise = this.transformWithCache({stylesheetLocation: bcdui.core.transformators.xsltToSefJson(this.stylesheetNode.URL), sourceDoc: sourceDoc});
  ...
  XSLTProcessor.prototype.getStylesheet = async function(sefUrl) {
    // check cache
    if (window.XSLTProcessor.sefCache.has(sefUrl))
      return window.XSLTProcessor.sefCache.get(sefUrl);
...
// TODO    window.XSLTProcessor.sefCache.set(sefUrl, promise);
    return promise;
  }
  ...
  XSLTProcessor.prototype.transform = function( args ) {
    ...
    let that = this;
    this.transformToDocument(args.input)
      .then(result => 
        { args.callBack(result); 
      })
      .catch(err => {
        console.error('Something went wrong:', err, that);
      });;
  }
