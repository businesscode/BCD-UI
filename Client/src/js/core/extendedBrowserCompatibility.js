...
//-----------------------------------------------------------------------------
  ...
  XSLTProcessor.prototype.transformToDocument = function(sourceDoc)
  {
    // saxonjs works with local-name xml:base and not namespace xml + base and complains about it in source documents 
    Array.from(sourceDoc.selectNodes("//*[@*[local-name() = 'xml:base']]")).forEach(function(x) { x.removeAttribute("xml:base"); });
    ...
    // if we have a stylesheetNode which was generated by a xslt transformation or dynamically e.g. as an inline model, we need to do a costy xslt3 transformation
      ...
      Array.from(this.stylesheetNode.selectNodes("//*[local-name()='import']")).forEach(function(e) {
        ...
        // if no baseUrl is available, imports need to be fully qualified
        if (baseUrl == "about:blank" && !importPath.startsWith(bcdui.contextPath))
      ...
      Array.from(this.stylesheetNode.selectNodes("//xsl:variable[contains(@select, 'document(')]/@select")).forEach(attr => {
        ...
        // if no baseUrl is available, imports need to be fully qualified
        if (baseUrl == "about:blank" && !uri.startsWith(bcdui.contextPath))
          throw new Error(`import '${attr.value}' in ${e._saxonBaseUri} needs to be prefixed with 'bcduicp://'`,{ cause: e });
      ...
      // finally kill possible xml:base attributes and add a valid one
      Array.from(this.stylesheetNode.selectNodes("//*[@*[local-name() = 'xml:base']]")).forEach(function(x) { x.removeAttribute("xml:base"); });
