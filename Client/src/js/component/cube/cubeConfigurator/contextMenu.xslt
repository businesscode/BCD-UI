...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="isCollapsable" select="starts-with($statusModelLayout/@expandCollapseCells, 'collapse') or $statusModelLayout/@expandCollapseCells='expand'"/>
    ...
    <ContextMenu>
        ...
        <xsl:when test="$contextType='ColDimensionMember'">
          <ContextMenuEntryGroup caption="Cell Actions" >
            ...
            <xsl:if test="not($isVdm)">
              <TwoColumns>
                <Entry caption="Hide" data-bcd-action="hideColDim">
                  <JavaScriptAction>
                ...
                <Entry caption="Hide all" data-bcd-action="hideAllColDim">
                  <JavaScriptAction>
                    var levelId = bcdui._migPjs._$(this.eventSrcElement).closest("tr").attr("levelId");
                    if( bcdui._migPjs._$(this.eventSrcElement).get(0).firstChild )
                ...
                <Entry caption="Exclude (req. apply)" data-bcd-action="excludeColDim">
                  <JavaScriptAction>
                    var levelId = bcdui._migPjs._$(this.eventSrcElement).closest("tr").attr("levelId");
                    if( bcdui._migPjs._$(this.eventSrcElement).get(0).firstChild )
                ...
                <Entry caption="Exclude all (req. apply)" data-bcd-action="excludeAllColDim">
                  <JavaScriptAction>
                    var levelId = bcdui._migPjs._$(this.eventSrcElement).closest("tr").attr("levelId");
                    if( bcdui._migPjs._$(this.eventSrcElement).get(0).firstChild )
            ...
            <Entry caption="Show all values for this level" data-bcd-action="showAllColDim">>
              <JavaScriptAction>
                var levelId = bcdui._migPjs._$(this.eventSrcElement).closest("tr").attr("levelId");
        ...
        <xsl:when test="$contextType='RowDimensionMember'">
          <ContextMenuEntryGroup caption="Cell Actions" >
            <xsl:if test="not($isVdm)">
              <ContextMenuSubHeader caption="Hide &amp; Exclude Dimension Members"/>
              <TwoColumns>
                <Entry caption="Hide" data-bcd-action="hideRowDim">
                  <JavaScriptAction>
                ...
                <Entry caption="Hide all" data-bcd-action="hideAllRowDim">
                  <JavaScriptAction>
                    var levelId = bcdui.factory.objectRegistry.getObject("bcdColIdent").value;
                    if( bcdui._migPjs._$(this.eventSrcElement).get(0).firstChild )
                ...
                <Entry caption="Exclude (apply)" data-bcd-action="excludeRowDim">
                  <JavaScriptAction>
                    var levelId = bcdui.factory.objectRegistry.getObject("bcdColIdent").value;
                    if( bcdui._migPjs._$(this.eventSrcElement).get(0).firstChild )
                ...
                <Entry caption="Exclude all (apply)" data-bcd-action="excludeAllRowDim">
                  <JavaScriptAction>
                    var levelId = bcdui.factory.objectRegistry.getObject("bcdColIdent").value;
                    if( bcdui._migPjs._$(this.eventSrcElement).get(0).firstChild )
              ...
              <Entry caption="Show all values for this level" data-bcd-action="showAllRowDim" data-total-id="{$bcdDimensionInner}">
                <JavaScriptAction>
                  var levelId = bcdui.factory.objectRegistry.getObject("bcdColIdent").value;
      ...
      <ContextMenuEntryGroup caption="General Options" >
        ...
        <TwoColumns>
          ...
          <Entry caption="Measure (all dim)" data-bcd-action="addColumnMeasure">
          ...
          <Entry caption="Measure (row dim)" data-bcd-action="addRowMeasure">
        ...
        <xsl:if test="not($isCollapsable)">
          <Entry data-bcd-action="toggleAllTotals">
            <xsl:attribute name="caption">
              <xsl:choose>
        ...
        <Entry caption="Show all hidden values" data-bcd-action="showAllHidden">
          <JavaScriptAction>
            var levelId = bcdui.factory.objectRegistry.getObject("bcdColIdent").value;
  ...
  <xsl:template name="columnSort">
    <xsl:if test="not($isCollapsable)">
      ...
      <TwoColumns>
        <Entry caption="Ascending" data-bcd-action="sortAscending" data-is-dim="{boolean(not($measure))}">
          <JavaScriptAction>
        ...
        <Entry caption="Descending" data-bcd-action="sortDescending" data-is-dim="{boolean(not($measure))}">
          <JavaScriptAction>
        ...
        <xsl:if test="$statusModelLayout/@manualSort='true'">
          <Entry caption="Clear sorting" data-bcd-action="clearSorting" data-is-dim="{boolean(not($measure))}">
            <JavaScriptAction>
  ...
  <xsl:template name="sortDimByMeas">
    ...
    <xsl:if test="($bcdDimensionMostInner = $bcdDimension) or ($statusModelLayout//dm:LevelRef[@total!='' and @bRef=$bcdDimensionInner] and not($statusModelLayout/cube:Hide//f:Expression[@bRef=$bcdDimensionInner])) and count($statusModelLayout//cube:Measures/*/*) != 0">
      ...
      <xsl:if test="$statusModelLayout//dm:LevelRef[@bRef=$bcdDimension]/@sortBy and $statusModelLayout//dm:LevelRef[@bRef=$bcdDimension]/@sort='ascending'">
        <Entry caption="Clear Sorting" data-bcd-action="clearSortDimByMeas" data-is-col-dim="{$isColDim}">
  ...
  <xsl:template name="sortDimByMeasInner">
    ...
    <TwoColumns>
      <xsl:for-each select="$wrsModel/wrs:Wrs/wrs:Header/wrs:Columns/wrs:C[@valueId and not(@valueId=preceding-sibling::wrs:C/@valueId) and (not($isColDim) or contains(@id,'|'))]">
        ...
        <Entry caption="{$caption}" data-bcd-action="sortDimByMeas" data-direction="{$direction}" data-sort-by="{@valueId}" data-is-col-dim="{$isColDim}">
  ...
  <xsl:template name="cumulate">
    ...
    <TwoColumns>
      <Entry caption="Cumulate row" data-bcd-action="cumulateRow">
      ...
      <Entry caption="Cumulate colunm" data-bcd-action="cumulateCol">
    ...
    <xsl:if test="boolean($measure/@cumulateCol) or boolean($measure/@cumulateRow)">
      <Entry caption="Clear cumulation" data-bcd-action="clearCumulation">
  ...
  <xsl:template name="totals">
    ...
    <xsl:if test="not($isCollapsable)">
      ...
      <xsl:if test="$statusModelLayout//dm:LevelRef[@total!='' and @bRef=$bcdDimensionInner] and not($statusModelLayout/cube:Hide//f:Expression[@bRef=$bcdDimensionInner and @op='!=' and @value='&#xE0F0;1'])">
        <Entry caption="Hide total values for this level" data-bcd-action="hideTotal" data-is-col-dim="{$isColDim}" data-level-id="{$bcdDimensionInner}" data-outer-level-id="{$colHead/@id}">
          <xsl:choose>
      ...
      <xsl:if test="$statusModelLayout//dm:LevelRef[@total!='' and @bRef=$bcdDimensionOuter] and not($statusModelLayout/cube:Hide//f:Expression[@bRef=$bcdDimensionOuter and @op='!=' and @value='&#xE0F0;1'])">
        <Entry caption="Hide grand total values" data-bcd-action="hideGrandTotal" data-level-id="{$bcdDimensionOuter}">
          <JavaScriptAction>
      ...
      <xsl:if test="$statusModelLayout//dm:LevelRef[(not(@total) or @total='') and @bRef=$bcdDimensionInner] or $statusModelLayout/cube:Hide//f:Expression[@bRef=$bcdDimensionInner and @op='!=' and @value='&#xE0F0;1']">
        <Entry caption="Show total values for this level" data-bcd-action="showTotal" data-level-id="{$bcdDimensionInner}" data-is-col-dim="{$isColDim}">
        <xsl:choose>
          <xsl:when test="$isColDim">
              <JavaScriptAction>
      ...
      <xsl:if test="$statusModelLayout//dm:LevelRef[(not(@total) or @total='') and @bRef=$bcdDimensionOuter] or $statusModelLayout/cube:Hide//f:Expression[@bRef=$bcdDimensionOuter and @op='!=' and @value='&#xE0F0;1']">
        <Entry caption="Show grand total values" data-bcd-action="showGrandTotal" data-level-id="{$bcdDimensionOuter}">
          <JavaScriptAction>
  ...
  <xsl:template name="detailExport">
    <ContextMenuSubHeader caption="Detail Export" data-bcd-action="detailExport"/>
