...
___
==== Release 6_0_2 ...in progress

.Client
* Added option to use BCD-UI as JavaScript modules like `import { AutoModel, Renderer } from "../bcdui/js/modules/core.js"`
* Added SaxonJS2 option to be used instead of browser native XSLT
    - BCD-UI now comes with sef files at /bcdui/sef/. They are automatically loaded if /bcdui/**/*.xslt is addressed
    - Projects can use Docu/development/bcdui_saxon_sef_generate.gradle to have sef files created. From the setup BCD-UI knows where to find them if a project XSLT is loaded
* Switched to JavaScript HtmlBuilder per default

.Server:
* BindingSet: Added `bnd:BindingDefaults` for inheriting BindingItem attributes defaults based on their id
* New feature for trusted machine clients to access the server with one-request-sessions, see package de.businesscode.bcdui.subjectsettings.services
+
[source,xml]
----
<!-- Sample: Unless overwritten in a BindingSet, this applies to all country_code BindingItems in all BindingSets -->
<bnd:BindingDefaults xmlns:bnd="http://www.businesscode.de/schema/bcdui/bindings-1.0.0">
  <bnd:C id="country_code" caption="Country Code" type-name="VARCHAR"/>
</bnd:BindingDefaults>
----
* Wrq: All attributes on `wrq:Select/wrq:Columns/wrq:C`, even custom ones, survive the roundtrip to `wrs:Header/wrs:Columns/wrs:C`
* BindingSet: Custom elements set on `bnd:BindingSet/bnd:C` can be delivered to the client at `wrs:Header/wrs:Columns/wrs:C`
    - If listed in web.xml bcdui/bnd/wrsAttributes for each request
    - If listed in web.xml bcdui/bnd/wrsMetaAttribute only for metadata request, i.e. @rowStart > @rowEnd

.Incompatibilities:
* In web.xml, add new env entries `bcdui/bnd/wrsAttributes` and `bcdui/bnd/wrsMetaAttribute`, see `Server/configFiles/WebContent/WEB-INF/web.xml` for backward compatible defaults
* If you xsl:import, the document() paths in these need to be relative to the including XSLT, was relative to the included. So you will have to overwrite this, like this then importing htmlBuilder.xslt
  ````xml
    <xsl:variable name="sqlTypesDoc" select="document('../../../../bcdui/xslt/renderer/sqlTypes.xml')"/>
  ````
  If that is not done, an error like `/bcdui/sef/xslt/xslt_transform.sef.json XError: xsl:stylesheet lacks @version` may show up
  because the document is tried to be loaded from the wrong place and could not be parsed
---.
...
* BindingSet: Added bnd:BindingDefaults for inheriting BindingItem attribute defaults based on their id
+
[source,xml]
----
<!-- Unless overwritten in a BindingSet, this applies to all country_code BindingItems in all BindingSets -->
<bnd:BindingDefaults xmlns:bnd="http://www.businesscode.de/schema/bcdui/bindings-1.0.0">
  <bnd:C id="country_code" caption="Country Code" type-name="VARCHAR"/>
</bnd:BindingDefaults>
----
...
** If a template is selected and then manually modified later, cube:Layout/@name and @description attributes are appended with ' - '+bcd_Edit_Modified
