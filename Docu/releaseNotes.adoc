...
.Incompatibilities:
** Move shiro settings from web.xml to WEB-INF/shiro.ini, add <listener><listener-class>org.apache.shiro.web.env.EnvironmentLoaderListener</listener-class></listener> before shiro filter, update shiro filter from org.apache.shiro.web.servlet.IniShiroFilter to org.apache.shiro.web.servlet.ShiroFilter
** Add security features regarding cookies to context.xml and web.xml
** Add SubjectFilters to bcd_virtualfilesystem
...
** getTableName() was renamed to getTableReference()
and needs also to be updated sql expression strings compiled with SQLEngine::transform()  "... $k.getTableName()..." -> $k.getTableReference() or simply $k
** TextToUrl servlet returns found value in an XML element now
** removed SubjectSettingsFilter, bcdui.config.security, client sided setting is done now via SubjectPreferences with bcdui/conf/subjectPreferences.xml as config.
* ensure adding bcd_userId:userId and bcd_i18n:lang to subjectSettings.xml

.Server:
<<<<<<< HEAD
* allow client sided setting of subject preferences, using config from bcdui/conf/subjectPreferences.xml
* introduce UserSettingsDefaults to subjectSettings.xml to specify user default permissions (add de.businesscode.bcdui.subjectsettings.SubjectPreferences to your realms)
* added document uploader classes
=======
* Wrq does support CTE common table expressions, joins and unions now
* Allowing client sided setting of bcdUserBean values via subjectAttributeManager servlet. ${bcdUserBean:name} in urls will be replaced and forwarded. Server sided read/write security via SubjectFilterType "bcdUserBean:name".
* Introduced UserSettingsDefaults to subjectSettings.xml to specify user default permissions (add de.businesscode.bcdui.subjectsettings.DefaultSettingsRealm to your realms)
* Database syntax does fuly support MySQL and Redshift now
...
* no need to add bcd__sidebar-left-collaps-toggle etc divs/classes. Specify bcdEffectCollapse class at your bcd__sidebar-*__inner element
